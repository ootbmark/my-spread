...
class WeeklyAlert extends Command
{
    const SUB_DAYS = 14;
     ...
     * @var string
     */
    protected $signature = 'weekly_alert';
     ...
     * @var string
     */
    protected $description = 'Send weekly alerts';
    ...
    public function handle()
    {
        ...
        foreach ($threads as $thread) {
            ...
            $all_groups[$thread->group_id]['discussions'][] = $thread;
            $thread->is_weekly_sent = 1;
            $thread->save();
        }
        ...
        foreach ($users as $user) {
            ...
            if (count($groups)) {
                ...
                $subject = 'Weekly Roundup: ' . count($groups) . ' Discussion' . (count($groups) > 1 ? 's' : '');
                try {
            ...
            if (count($reply_groups)) {
                ...
                $subject = 'Weekly Roundup: ' . $subject_resp_count . ' Response' . ($subject_resp_count > 1 ? 's' : '') . ' in ' . count($reply_groups) . ' Discussion Groups';
                try {
