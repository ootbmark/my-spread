...
{{--        </div>--}}
        ...
        <form class="quiz" action="{{ route('quiz-answer.update', $report->id) }}" method="post" id="dane_form">
            @csrf
            @method('PATCH')
            <div class="d-md-flex  @if(!count($groups)) justify-content-center @else justify-content-between @endif align-items-center mt-5  mb-2 quiz-container ">
                @if(count($groups))
                    <div class="">
                        ...
                        <select id="user_group" class="form-control mb-2" style="color: #555" name="group">
                            <option value="" disabled selected>Choose group</option>
                            @foreach($groups as $key => $group)

                                <option @if($report->group_id === $key) selected @endif value="{{ $key }}">{{ $group }}</option>
                            @endforeach
                        </select>
                ...
                <div class="d-md-flex">
                    <div class="text-center mr-md-5 d-flex flex-column align-items-center">
                        ...
                        <div class="d-flex align-items-center">
                            <div class="custom-radio">
                                <input id="hard" type="radio" name="status" value="high" class="radio-high"
                                       @if($report->status === 'high') checked @endif>
                                <label class="hard-color" for="hard">High</label>
                                <input id="medium" type="radio" name="status" value="medium" class="radio-medium"
                                       @if($report->status === 'medium') checked @endif>
                                <label class="medium-color" for="medium">Medium</label>
                                <input id="low" type="radio" name="status" value="low" class="radio-low"
                                       @if($report->status === 'low') checked @endif>
                    ...
                    <div class="text-center mr-md-5 d-flex flex-column align-items-center">
                        ...
                        <div class="d-flex align-items-center d-flex flex-column align-items-center">
                            <div class="custom-radio">
                                <input id="hard2" type="radio" name="status_effort" value="high" class="radio-high"
                                       @if($report->status_effort === 'high') checked @endif>
                                <label class="hard-color" for="hard2">High</label>
                                <input id="medium2" type="radio" name="status_effort" value="medium" class="radio-medium"
                                       @if($report->status_effort === 'medium') checked @endif>
                                <label class="medium-color" for="medium2">Medium</label>
                                <input id="low2" type="radio" name="status_effort" value="low" class="radio-low"
                                       @if($report->status_effort === 'low') checked @endif>
                    ...
                    <div class="text-center mr-0">
                        ...
                        <div class="d-flex align-items-center d-flex flex-column align-items-center">
                            <div class="custom-radio">
                                <input id="hard3" type="radio" name="priority" value="high" class="radio-high"
                                       @if($report->priority === 'high') checked @endif>
                                <label class="hard-color" for="hard3">High</label>
                                <input id="medium3" type="radio" name="priority" value="medium" class="radio-medium"
                                       @if($report->priority === 'medium') checked @endif>
                                <label class="medium-color" for="medium3">Medium</label>
                                <input id="low3" type="radio" name="priority" value="low" class="radio-low"
                                       @if($report->priority === 'low') checked @endif>
    ...
    c0,10.255-3.296,18.312-9.888,24.17C261.7,411.481,254.084,414.412,245.442,414.412z"/>
                        ...
                        @if($question->type === 'file')
                            <div
                                ...
                                <input type="hidden" value="file" name="questions[{{$question->id}}][type]">
                                @foreach($question->answers as $key => $answer)
                                    <div class=" custom-control custom-checkbox checkbox-image">
                                        <input type="checkbox" class="custom-control-input" value="{{ $answer->id }}" name="questions[{{$question->id}}][answer][ids][]"
                        ...
                        @elseif($question->type === 'multiple')
                            <div {{ $question->question_required ? 'data-required=1' : '' }}>
                                <input type="hidden" value="multiple" name="questions[{{$question->id}}][type]">
                                @foreach($question->answers as $key => $answer)
                                    <div class=" custom-control custom-checkbox">
                                        <input class="custom-control-input custom-checkbox" type="checkbox" name="questions[{{$question->id}}][answer][ids][]"
                                               {{ isset($answers[$question->id]) && in_array_field($answer->id, 'answer_id', $answers[$question->id]) ? 'checked' : '' }}
                        ...
                        @elseif($question->type === 'dropdown')
                            <div {{ $question->question_required ? 'data-required=1' : '' }}>
                                <input type="hidden" value="dropdown" name="questions[{{$question->id}}][type]">
                                <div class="form-group" >
                                    <select class="custom-select" data-id="{{ $question->id }}" aria-label="select" name="questions[{{$question->id}}][answer][id]">
                        ...
                        @elseif($question->type === 'radio')
                            <div {{ $question->question_required ? 'data-required=1' : '' }}>
                                <input type="hidden" value="radio" name="questions[{{$question->id}}][type]">
                                @foreach($question->answers as $answer)
                                    <div class=" custom-control custom-radio">
                                        <input class="custom-control-input" type="radio" name="questions[{{$question->id}}][answer][id]"
                        ...
                        @elseif($question->type === 'text')
                            ...
                            <div class="form-group" {{ $question->question_required ? 'data-required=1' : '' }}>
                                <input type="hidden" value="text" name="questions[{{$question->id}}][type]">
                                <input type="text" class="form-control text" name="questions[{{$question->id}}][answer][text]"
                        ...
                        @elseif($question->type === 'textarea')
                            <div class="form-group" {{ $question->question_required ? 'data-required=1' : '' }}>
                            <input type="hidden" value="textarea" name="questions[{{$question->id}}][type]">
                            <textarea class="form-control textarea" rows="3" data-id="{{ $question->id }}"
                                  aria-label="textarea" name="questions[{{$question->id}}][answer][text]">{{ Arr::exists($answers, $question->id) ? $answers[$question->id][0]['text'] : ''}}</textarea>
                                <small class="text-danger error" style="display: none">The field is required</small>
                            </div>
                        @elseif($question->type === 'circling')
                            <div class="circling" {{ $question->question_required ? 'data-required=1' : '' }}>
                                <input type="hidden" value="circling" name="questions[{{$question->id}}][type]">
                                <div class="d-flex">
                                    @foreach($question->answers as $key => $answer)
                                        @php
                                            $checked = $answer->quiz_answer()->where('quiz_report_id', $report->id)->exists();
                                        @endphp
                                        <div style="margin-right: 40px" class=" custom-control custom-checkbox">
                                            <input class="custom-control-input custom-checkbox" type="checkbox"
                                                   name="questions[{{$question->id}}][answer][ids][]"
                                                   data-type="circling"
                                ...
                                <textarea class="form-control textarea" rows="3" data-id="{{ $question->id }}"
                                          data-type="circling_text" name="questions[{{$question->id}}][answer][text]">{{ Arr::exists($answers, $question->id) ? $answers[$question->id][0]['text'] : ''}}</textarea>
                                    <small class="text-danger error" style="display: none">The fields is
                                        required</small>
                                </div>
                            </div>
                ...
                <div class="d-flex quiz-ipt">
                    <div class="quiz-ipt-left">
                        <input class="quiz-ipt-each form-control" type="text" name="focal_point" value="{{$report->focal_point}}"
                               placeholder="Oil Co Focal Point (e.g. Drilling Engr.)">
                        <input class="quiz-ipt-each form-control" type="text" name="action_party" value="{{$report->action_party}}"
                    ...
                    <div class="quiz-ipt-right">
                        <input class="datepicker quiz-ipt-each form-control" name="target_date" type="text" value="{{$report->target_date}}"
                               autocomplete="off"
                               placeholder="And by when:">
                        <input class="quiz-ipt-each form-control" type="text" name="business_partner" value="{{$report->business_partner}}"
                ...
                <div class="text-center mt-4">
                    <button type="submit" class="btn my-btn text-uppercase done">{{ __('Done') }}</button>
                </div>
            </div>
        </form>
